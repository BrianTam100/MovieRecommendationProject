...
const TelevisionDetails = ({ id }: Props) => {
  const [details, setDetails] = useState<Details | null>(null);
  const [cast, setCast] = useState<Actors[]>([]);

  const API_KEY = 'YOUR_API_KEY'; // Ensure your API key is correct.
  ...
  return (
    <div className="bg-gray-800 min-h-screen p-8 text-white">
      ...
      <h2 className="text-1xl font-bold ml-[20%]">
        {details.number_of_seasons} Seasons | {details.first_air_date.slice(0, 4)} - {details.in_production !== true ? details.last_air_date.slice(0, 4) : 'Present'} |{' '}
